'use strict';
/*this is a variation on Walter Higgins' mazegen with a few small touches... for helping create underground mazes
  try: down(5).airmaze(7,5);
...
Drone.extend( function airmaze( width, length ) {
...
function maze_draw(maze_string, d) {
  ...
  for (var j = 0; j < maze_string.length; j += 2) {
    ...
    case '  ':
      d.box(0).fwd(2); // Make sure to empty this position
      break;
    case '\r\n':
      d.move('maze-start');
    ...
    default: // '+ ', '+-', '--', '| '
      if (j == 0) {
        d.box(blocks.air,1,6,1); // highlight the maze entry and exit
      } else if (j == maze_string.length - 4) {
        d.box(blocks.air,2,6,2);
      } else {
        d.box(blocks.air,2,6,2);
      }
      d.fwd(2);
